...
from models.base import ImplicitSurface, NeRF, RadianceNet, TinyImplicitSurface, TinyRadianceNet, ExplicitGrid
...
class VolSDF(nn.Module):
    def __init__(self,
        ...
        self.implicit_surface, self.radiance_net = get_surface(
            sdf_rep, surface_cfg, radiance_cfg, W_geo_feat, input_ch, obj_bounding_radius, )
        # if use_tinycuda:
        #     self.implicit_surface = TinyImplicitSurface(
        ...
        # if use_tinycuda:
        #     self.radiance_net = TinyRadianceNet(
...
def get_surface(sdf_rep, surface_cfg, radiance_cfg, W_geo_feat, input_ch, obj_bounding_radius,):
    if sdf_rep == 'net':
        ...
        implicit_surface = TinyImplicitSurface(
            W_geo_feat=W_geo_feat, 
            **surface_cfg)
    elif sdf_rep == 'grid':
        implicit_surface = ExplicitGrid(1, **surface_cfg)
    else:
        raise NotImplementedError(sdf_rep)
    
    if W_geo_feat < 0:
        W_geo_feat = implicit_surface.W

    if sdf_rep == 'net':
        radiance_net = RadianceNet(
            W_geo_feat=W_geo_feat, **radiance_cfg)
    elif sdf_rep == 'tiny':
        radiance_net = TinyRadianceNet(
            W_geo_feat=W_geo_feat, **radiance_cfg)
    elif sdf_rep == 'grid':
        radiance_net = ExplicitGrid(3, **radiance_cfg)
    else:
        raise NotImplementedError
    return implicit_surface, radiance_net
...
def volume_render(
    ...
    **dummy_kwargs  # just place holder
    ...
    """
    # print('near', near, far, obj_bounding_radius,)
    ...
    def render_rayschunk(rays_o: torch.Tensor, rays_d: torch.Tensor):
        ...
        pts_coarse = rays_o[..., None, :] + rays_d[..., None, :] * d_coarse[..., :, None]
        pts_fine = rays_o[..., None, :] + rays_d[..., None, :] * d_fine[..., :, None]
        # print('min max of coarse fine points', pts_coarse.min(), pts_coarse.max(), pts_fine.min(), pts_fine.max())
        ...
        if detailed_output:
            ...
            # ret_i['grad_theta'] = nablas[:nablas.shape[0]//2]
            # ret_i['grad_theta_nei'] = nablas[nablas.shape[0]//2:]
