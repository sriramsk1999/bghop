...
class Logger(object):
    ...
    def setup_monitoring(self, monitoring, monitoring_dir):
        ...
        if monitoring == "tensorboard":
            ...
            raise print('Monitoring tool "%s" not supported!' % monitoring)
    ...
    def add_depth(self, imgs, class_name, it):
        outdir = os.path.join(self.img_dir, class_name)
        outfile = os.path.join(outdir, "{:08d}_{}.png".format(it, self.rank))
        if self.is_master:
            os.makedirs(os.path.dirname(outfile), exist_ok=True)
        ...
        # imgs = imgs / 2 + 0.5
        imgs = torchvision.utils.make_grid(imgs)[0]
        print(imgs.shape, imgs.min(), imgs.max())
        plt.imsave(outfile, imgs.detach().cpu().squeeze(), cmap="viridis")

        if is_master() and self.wandb:
            # wandb.log({class_name: wandb.Image(imgs)}, step=it)
            wandb.log({class_name: wandb.Image(outfile)}, step=it)
    ...
    def add_meshes(self, name, mesh_file, it):
        if is_master() and self.wandb and not mesh_file.endswith(".ply"):
            wandb.log(
                {name: wandb.Object3D(open(osp.join(self.log_dir, mesh_file)))}, step=it
            )
